...
import captureScreenshot from './scripts/actions/captureScreenshot'
import lastUsedTab from './scripts/actions/lastUsedTab'
...
let selectTab = (direction) => {
  browser.tabs.query({ currentWindow: true }).then(function(tabs) {
    ...
    browser.tabs.query({ currentWindow: true, active: true }).then(function(currentTabInArray) {
      ...
      switch (direction) {
        case 'next':
          jumpToNextTab(true, currentTabIndex, tabs)
          break
        case 'previous':
          jumpToNextTab(false, currentTabIndex, tabs)
          break
        case 'next-force':
          forceJumpToNextTab(true, currentTabIndex, tabs)
          break
        case 'previous-force':
          forceJumpToNextTab(false, currentTabIndex, tabs)
          break
        case 'first':
  ...
  browser.tabs.query({ currentWindow: true }).then(function(tabs) {
    ...
    browser.tabs.query({ currentWindow: true, active: true }).then(function(currentTabInArray) {
      ...
      if (currentTab.groupId != -1) {
        browser.tabGroups.update(currentTab.groupId, { collapsed: true })
      }
    })
  })
}
...
let handleAction = (action, request = {}) => {
  ...
  if (action === 'cleardownloads') {
    ...
    selectTab('previous')
  } else if (action === 'nexttabforced') {
    selectTab('next-force')
  } else if (action === 'prevtabforced') {
    selectTab('previous-force')
  ...
  } else if (action === 'collapsecurrentgroup') {
